...
if upload is not None:
    ...
    # Use session state data for all operations
    ...
    if st.checkbox("Preview Dataset"):
        ...
        if st.checkbox("Do you want to delete any columns?"):
            ...
            if st.button("Delete Selected Columns"):
                ...
                st.session_state.data = data  # Update session state with the new data
    ...
    # 8.1 View dataset info
    if st.checkbox("Again View Dataset Info"):
        buffer = io.StringIO()
        data.info(buf=buffer)
        s = buffer.getvalue()
        st.text(s)
